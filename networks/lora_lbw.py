# LoRA network module with SDXL LoRA Block Weight support for sdxl_gen_img.py.
# This file is copied from lora.py for image-generation use, so training can keep using networks.lora unchanged.
# reference:
...
class LoRAModule(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        delta = lx * self.multiplier * scale * self.lbw_multiplier
...
class LoRAInfModule(LoRAModule):
    ...
    def merge_to(self, sd, dtype, device):
        ...
        if len(weight.size()) == 2:
            # linear
            weight = weight + self.multiplier * self.lbw_multiplier * (up_weight @ down_weight) * self.scale
            ...
            weight = weight + self.multiplier * self.lbw_multiplier * conved * self.scale
    ...
    def get_weight(self, multiplier=None):
        ...
        if len(down_weight.size()) == 2:
            # linear
            weight = multiplier * self.lbw_multiplier * (up_weight @ down_weight) * self.scale
        elif down_weight.size()[2:4] == (1, 1):
            # conv2d 1x1
            weight = (
                multiplier
            ...
            # conv2d 3x3
            conved = torch.nn.functional.conv2d(down_weight.permute(1, 0, 2, 3), up_weight).permute(1, 0, 2, 3)
            weight = multiplier * self.lbw_multiplier * conved * self.scale
...
def parse_sdxl_lbw_weights(weights: Union[str, Sequence[float], None]) -> Optional[List[float]]:
    if weights is None:
        return None
    if isinstance(weights, str):
        ...
        if weights == "":
            return None
        weights = SDXL_LBW_PRESETS.get(weights.upper(), weights)
        values = [float(v.strip()) for v in weights.split(",")]
    else:
        values = [float(v) for v in weights]

    if len(values) != SDXL_LBW_NUM_WEIGHTS:
        raise ValueError(
            f"SDXL LoRA Block Weight requires {SDXL_LBW_NUM_WEIGHTS} values but {len(values)} values are given "
            f"/ SDXL LoRA Block Weightは{SDXL_LBW_NUM_WEIGHTS}個指定してください"
        )
    return values


def get_sdxl_lbw_index(lora_name: str) -> Optional[int]:
    if lora_name.startswith(LoRANetwork.LORA_PREFIX_TEXT_ENCODER):
        return 0  # BASE
    if not lora_name.startswith(f"{LoRANetwork.LORA_PREFIX_UNET}_"):
        return None

    name = lora_name[len(f"{LoRANetwork.LORA_PREFIX_UNET}_") :]
    if name.startswith("input_blocks_"):
        block = int(name.split("_")[2])
        return {4: 1, 5: 2, 7: 3, 8: 4}.get(block)
    if name.startswith("middle_block_"):
        return 5
    if name.startswith("output_blocks_"):
        block = int(name.split("_")[2])
        return {0: 6, 1: 7, 2: 8, 3: 9, 4: 10, 5: 11}.get(block)
    return None
...
class LoRANetwork(torch.nn.Module):
    ...
    def set_lbw_weights(self, weights: Union[str, Sequence[float], None]):
        lbw_weights = parse_sdxl_lbw_weights(weights)
        for lora in self.text_encoder_loras + self.unet_loras:
            if lbw_weights is None:
