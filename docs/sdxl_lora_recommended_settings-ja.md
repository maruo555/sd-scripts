...
| `--avg_cp_mode`（独自） | live | avg_cp の動作モード | `live` は従来どおり平均重みをそのまま学習へ反映 `shadow` は raw と center を比較してログと best 保存だけ行う `promote` は比較結果が良いときだけ center を次 epoch に採用 動作確認や観測なら `shadow`、安全に条件付き導入したいなら `promote`、従来運用をそのまま使うなら `live` |
