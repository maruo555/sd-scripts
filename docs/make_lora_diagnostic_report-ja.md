...
| `diagnostics` | `grad` / `dq` / `lora` の要約値から派生 | ヒューリスティクス診断スコアと判定一覧 |
| `charts` | `grad` / `dq` / `group_loss` / `lora_trend` から派生 | HTML描画用のグラフペイロード |
...
- `RankDim`, `RankSatWMean`, `RankSatP50`, `RankSatP95`, `RankSatMax`
