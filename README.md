...
| `--rank_log`<br>`--rank_log_every`<br>`--rank_log_mode`<br>`--rank_log_file` | 無効 (`every=100`, `mode=summary`) | LoRA重みから推定した rank 飽和指標（`RankSat*`, `RankTop1P95`, `RankEnergySum`）を独立ログとして出力。`--dq_delta_log` とは独立して動作し、dq_delta 無効時でも記録可能。 | 仕様は [docs/rank_log_spec-ja.md](docs/rank_log_spec-ja.md) を参照。 |
