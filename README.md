...
| `--group_loss_log`, `--group_loss_ema_beta`,<br>`--group_loss_log_every_n_steps`, `--group_loss_epoch_summary` | 無効 (`beta=0.98`, `every=100`) | subsetの `group` 単位で loss をオンラインEMA集計し、step/epoch CSVを出力。学習配分の偏り観測に使う。 | `batch_size=1` 専用。`skip_grad_norm` でskipされたstep・NaN/Inf stepは集計除外。詳細: [docs/group_loss_ema_logging-ja.md](docs/group_loss_ema_logging-ja.md) |
