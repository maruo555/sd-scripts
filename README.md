...
| `--group_lr_auto` と `--group_lr_auto_*` | 無効 | epoch境界で group別loss(EMA) から boost-only のLR倍率を自動更新し、次epochの該当group学習stepに適用する。`N=3+`はmedian基準、`N=2`はmin基準、`N<=1`は全1.0へ戻す。 | `--group_loss_log` と `--group_loss_epoch_summary` 必須。`gradient_accumulation_steps=1` かつ 単一process限定。詳細: [docs/group_loss_ema_logging-ja.md](docs/group_loss_ema_logging-ja.md) |
