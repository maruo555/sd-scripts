...
| `--dropout_sched 'k0:v0,k1:v1,...'`<br>`--dropout_sched_log_every <int>` | `None` / `0` | dropout の**係数**スケジュール。学習進行率 `k` (0〜1) に対し係数 `v` を指定し、区間は**線形補間**。最後の点以降は最後の値で保持。<br>係数は `--network_dropout` / `--network_args "rank_dropout=..."` / `--network_args "module_dropout=..."` の**指定値に乗算**して適用。未指定の対象はそのまま。 | 書式は `--dq_delta_bits_sched` と同じ。<br>`--dropout_sched_log_every` を指定すると `output_dir` に `dropout_sched+<name>.txt` を作成し、step/係数/各dropoutを記録。 |
