...
| `--dq_delta_triton_fused_up_mode {off,c0}`<br>`--dq_delta_triton_fused_up_scope {unet,te,both}`<br>`--dq_delta_triton_fused_up_diagnostics` | `off`<br>`unet`<br>無効 | **rank 4専用 Quantized LoRA-Up（C0）**。LoRA-Up、channel RMS scale、8bit stochastic fake quantを専用Triton pipelineで処理する。`mode=c0`には`--dq_delta_use_triton`が必要。`scope`はC0を試す範囲で、実効対象は`--dq_delta_scope`との共通部分。`diagnostics`はshape、coverage、fallback理由を収集するため、正式な速度測定では無効を推奨。 | C0が使えないrank・shape・dtype・GPU・統計stepは、既存Triton A/BまたはPyTorchへfallback。gradient checkpointing中と`torch.no_grad()`ではC0を使わない。C0の適用条件と検証方法は [C0ガイド](docs/rank4_quantized_lora_up-ja.md)、scope修正の経緯と旧実動作の再現設定は [scope互換性の説明](docs/dq_delta_mechanism-ja.md#scope-semantics-version-2) を参照。 |
| `--dq_delta_log`<br>`--dq_delta_auto_range_mul` | 無効 | **dq_delta のログ**と**range_mul の自動調整**。<br>• `--dq_delta_log` = dq_delta の統計ログを出力。<br>• `--dq_delta_auto_range_mul` = clip_rate を見て range_mul を自動調整。<br>• `--dq_delta_auto_scope {unet,te,both}` = auto統計対象。未指定時は`--dq_delta_scope`を継承し、その部分集合だけを指定可能。<br>• `--dq_delta_auto_preset {default,clip_rate_high,clip_rate_high_narrow,clip_rate_mid,clip_rate_low,clip_rate_low_auto}` = auto 調整プリセット（clip_low/high のみ切替）。`clip_rate_low_auto` はlow帯から開始し、誤差負荷が高い場合はmid帯へ一方向に退避。既定では進捗0.90まで切替を許可。<br>• `--dq_delta_auto_use_raw` = auto 判定に clip_rate_raw も使う（既定 OFF）。mul の変化がなだらかになるため、`clip_rate_high_narrow` / `clip_rate_low` を追加。<br>• `--dq_delta_auto_init_range_mul_from_band` = clip帯中心から range_mul 初期値を自動算出（auto 有効時、`stat=rms` のみ）。<br>• `--dq_delta_auto_warmup` / `--no-dq_delta_auto_warmup` = 序盤は EMA/ログのみ更新し、range_mul を変更しない（既定 ON）。<br>• `--dq_delta_auto_warmup_updates <int>` = warmup 回数の上書き（0=内部デフォルト）。<br>プリセット例: `--dq_delta_auto_range_mul --dq_delta_auto_preset clip_rate_high`<br>発動条件:<br>• `--dq_delta_log` は dq_delta 有効時のみ。<br>• `--dq_delta_auto_range_mul` は **bits モード + `stat=rms`** の時のみ有効（`--dq_delta_bits` または `--dq_delta_bits_sched` が必要）。 | 詳細は [docs/dq_delta_autotune_spec-ja.md](docs/dq_delta_autotune_spec-ja.md) を参照。 |
