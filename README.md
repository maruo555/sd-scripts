...
| `--group_loss_log`, `--group_loss_ema_beta`,<br>`--group_loss_log_every_n_steps`, `--group_loss_epoch_summary` | 無効 | subsetの `group` 単位で loss をオンラインEMA集計し、step/epoch CSVを出力。stepログは全step記録し、`every_n_steps` でまとめてflush。学習配分の偏り観測に使う。 | `batch_size=1` 専用。`skip_grad_norm` でskipされたstep・NaN/Inf stepは集計除外。既定: `group_loss_ema_beta=0.98`, `group_loss_log_every_n_steps=100`。詳細: [docs/group_loss_ema_logging-ja.md](docs/group_loss_ema_logging-ja.md) |
| `--group_lr_auto` と `--group_lr_auto_*` | 無効 | epoch境界で group別の進捗比（baseline基準）から boost-only のLR倍率を自動更新し、次epochの該当group学習stepに適用する。`N>=2` はmedian基準（`N==2` も中央値=2値平均）、`N<=1` は全1.0へ戻す。`ratio_ema>=threshold` のgroupのみ補正（既定 `threshold=1.10`）。 | `--group_loss_log` と `--group_loss_epoch_summary` 必須。`gradient_accumulation_steps=1` かつ 単一process限定。`--resume` / `--save_state` / `--save_state_on_train_end` / `--resume_from_huggingface` / `--save_state_to_huggingface` とは併用不可。詳細: [docs/group_loss_ema_logging-ja.md](docs/group_loss_ema_logging-ja.md) |
| `--avg_cp` | **False** | 有効化するとエポック間の LoRA 重みを平均して安定化（SWA/EMA 相当）。<br>  推奨プリセット：`--avg_cp --avg_window 4 --avg_begin 0.6 --avg_mode ema --avg_reset_stats`<br>  併用オプション:<br>  • `--avg_window <int>` 平均に使うチェックポイント数。<br>  • `--avg_begin <0-1>` 学習の進行率。ここを過ぎてから窓に溜め始め、<br>    窓が満杯になったエポック以降で平均が発動。<br>    流れ：①エポック末に raw ckpt を保存 → ②窓が満杯なら平均 → ③平均後の<br>    重みで次エポックを開始。<br>  • `--avg_mode {uniform,ema,metric}` 平均方法。`uniform`=等重み, `ema`=指数移動平均,<br>    `metric`=指標重み付き（準備中）。<br>  • `--avg_reset_stats / --no_avg_reset_stats` 平均直後に Optimizer のモーメンタム統計を<br>    安全にリセットして発散を防ぐかどうか。 | LoRAを学習後に最後の数エポックを平均するといい結果になるという話があるので、学習後半から平均しながら学習を進められるるようにする検証用。<br>詳細: [docs/avg_cp_behavior-ja.md](docs/avg_cp_behavior-ja.md) |
