...
| `--fp16_safe_norms`<br>`--fp16_safe_norms_mode {off,strict,native_accum}` | `False` / `off` | 縮約系（LayerNorm/GroupNorm/Softmax）を安全に計算。従来フラグは全対象を明示的にfp32化する `strict`。`native_accum` はCUDA LayerNormだけfp16入出力＋内部fp32統計にしてcastを削減し、GroupNorm/Softmaxは `strict` のまま。 | `native_accum` はRTX 5080の短時間実学習で `strict` 比2.7〜6.0%高速。従来フラグとの互換性あり。詳細・測定条件: [docs/fp16_safe_norms-ja.md](docs/fp16_safe_norms-ja.md) |
